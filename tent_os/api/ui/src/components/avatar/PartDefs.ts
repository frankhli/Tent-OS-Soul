...
import { BRAND_COLORS, shadeColor } from './PartSystem';

function drawHeadBase(ctx: CanvasRenderingContext2D, _state: PartDrawContext) {
  const r = 34;
  const grad = ctx.createRadialGradient(-5, -10, 3, 0, 0, r);
  grad.addColorStop(0, '#fff8f0');
  grad.addColorStop(0.35, '#f5e6d8');
  grad.addColorStop(0.85, '#e8d4c4');
...
function drawEye(ctx: CanvasRenderingContext2D, side: 'left' | 'right', state: PartDrawContext) {
  ...
  irisGrad.addColorStop(0, '#74c69d');
  irisGrad.addColorStop(0.5, asleep ? '#64748b' : '#2d6a4f');
...
function drawBlush(ctx: CanvasRenderingContext2D, side: 'left' | 'right', state: PartDrawContext) {
  ...
  grad.addColorStop(0, `rgba(255, 170, 180, ${amount * 0.55})`);
  grad.addColorStop(1, 'rgba(255, 170, 180, 0)');
  ctx.fillStyle = grad;
  ctx.beginPath();
