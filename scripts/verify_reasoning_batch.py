#!/usr/bin/env python3
"""
快速验证：Tool Loop reasoning 批量发送是否生效
"""
...
async def verify():
    ...
    session_id = f"verify_reasoning_{int(time.time())}"
    reasoning_chunks = []
    content_chunks = []
    completed = False
    ...
    # 发送一个会触发 Tool Loop 的问题
    await ws.send(json.dumps({
        "type": "chat.message",
        "payload": {
            "session_id": session_id,
            "user_id": USER_ID,
            "content": "执行 ls 命令看看当前目录下有什么",
        }
    }))
    ...
    if total_reasoning_msgs == 0:
        print("⚠️ 没有收到 reasoning 消息（可能没走 Tool Loop 或 silent 模式）")
    elif total_reasoning_msgs < 50 and avg_reasoning_chunk >= 60:
        print("✅ PASS: reasoning 批量发送生效！消息数显著减少，块大小合理。")
    elif total_reasoning_msgs < 100:
        print("🟡 PARTIAL: 有改善但可能还有优化空间")
    else:
        print("❌ FAIL: reasoning 仍逐字发送，消息数过多")
